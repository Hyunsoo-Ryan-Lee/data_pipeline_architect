...
task_info = EasyDict(
    dict(
        ...
        # 작업의 전체적인 정보
        execution_info = dict(
            execution_name = "D_SOURCEtoBRONZE_temp_dev", # [수정] 작업의 이름
        ...
        # 추출할 테이블 관련 내용
        extract_info = dict(
            query_location = "dev", # [수정] db/temp 폴더 내 최상위 디렉토리 이름
            query_file = "database_dev_a", # [수정] 이행 쿼리가 저장된 파일 이름
            database_name = "database_dev_a" # [수정] 이행 쿼리의 데이터베이스
        ),
        # 이행될 STORAGE 관련 내용
        load_info = dict(
